...
and desugar_formula env (f:term) : S.term =
  ...
  let desugar_quant (q:lident) b attr pats body =
    ...
    let with_pats env (names, pats) body =
      ...
      | _ ->
        ...
        in
        mk (Tm_meta (body, Meta_pattern (attr, names, pats)))
    in
    match tk with
      | Some a, k ->
 ...
 let push_quant
      (q:(list<AST.binder> * option<ident> * AST.patterns * AST.term) -> AST.term')
      (binders:list<AST.binder>)
      attr pats (body:term) =
    match binders with
    | b::(b'::_rest) ->
      let rest = b'::_rest in
      let body = mk_term (q(rest, attr, pats, body)) (Range.union_ranges b'.brange body.range) Formula in
      mk_term (q([b], attr, ([], []), body)) f.range Formula
    | _ -> failwith "impossible" in
  ...
  match (unparen f).tm with
    ...
    | QForall((_1::_2::_3), attr, pats, body) ->
      let binders = _1::_2::_3 in
      desugar_formula env (push_quant (fun x -> QForall x) binders attr pats body)

    | QExists((_1::_2::_3), attr, pats, body) ->
      let binders = _1::_2::_3 in
      desugar_formula env (push_quant (fun x -> QExists x) binders attr pats body)

    | QForall([b], attr, pats, body) ->
      desugar_quant C.forall_lid b attr pats body

    | QExists([b], attr, pats, body) ->
      desugar_quant C.exists_lid b attr pats body
