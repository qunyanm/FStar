...
and tc_maybe_toplevel_term env (e:term) : term                  (* type-checked and elaborated version of e            *)
  ...
  | Tm_meta(e, Meta_pattern(attr, names, pats)) ->
    let t, u = U.type_u () in
    let e, c, g = tc_check_tot_or_gtot_term env e t in
    let is_nopattern_attr attr = 
      match attr with
      | None -> false
      | Some x -> x.idText = "nopattern" in
    let pats = match pats with
      | [] -> 
        ...
        if Options.auto_patterns ()  && (not (is_nopattern_attr attr)) then 
    ...
    mk (Tm_meta(e, Meta_pattern(attr, names, pats))) None top.pos,
    c,
