    ...
    | Meta(env_m, m, r)::stack ->
      let m =
          ...
          | Meta_pattern (attr, names, args) ->
            Meta_pattern (attr,
                          names |> List.map (non_tail_inline_closure_env cfg env_m),
                ...
                then norm cfg env stack head
                  ...
                  | [] ->
                    ...
                    let m = match m with
                        | Meta_pattern (attr, names, args) ->
                          let names =  names |> List.map (norm cfg env []) in
                          Meta_pattern (attr, names, norm_pattern_args cfg env args)
                        | _ -> m in
...
and elim_delayed_subst_meta = function
  | Meta_pattern (attr, names, args) -> Meta_pattern(attr, List.map elim_delayed_subst_term names, List.map elim_delayed_subst_args args)
