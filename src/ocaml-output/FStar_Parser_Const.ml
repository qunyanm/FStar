...
let (mk_range_lid : FStar_Ident.lident) = pconst "mk_range" 
...
let (gen_reset : ((unit -> Prims.int) * (unit -> unit))) =
  ...
  let gen1 uu____879 = FStar_Util.incr x; FStar_Util.read x  in
...
let (sli : FStar_Ident.lident -> Prims.string) =
  fun l  ->
    let uu____918 = FStar_Options.print_real_names ()  in
    if uu____918
...
let (const_to_string : FStar_Const.sconst -> Prims.string) =
  fun x  ->
    ...
    | FStar_Const.Const_string (s,uu____947) -> FStar_Util.format1 "\"%s\"" s
    | FStar_Const.Const_bytearray uu____951 -> "<bytearray>"
    | FStar_Const.Const_int (x1,uu____960) -> x1
    | FStar_Const.Const_char c ->
        let uu____977 = FStar_String.op_Hat (FStar_Util.string_of_char c) "'"
           in
        FStar_String.op_Hat "'" uu____977
    | FStar_Const.Const_range r -> FStar_Range.string_of_range r
    | FStar_Const.Const_range_of  -> "range_of"
    | FStar_Const.Const_set_range_of  -> "set_range_of"
    | FStar_Const.Const_reify  -> "reify"
    | FStar_Const.Const_reflect l ->
        let uu____986 = sli l  in
        FStar_Util.format1 "[[%s.reflect]]" uu____986
  
let (mk_tuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____1004 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "tuple%s" uu____1004  in
      let uu____1007 = psnconst t  in FStar_Ident.set_lid_range uu____1007 r
...
let (is_tuple_constructor_lid : FStar_Ident.ident -> Prims.bool) =
  fun lid  ->
    let uu____1028 = FStar_Ident.text_of_id lid  in
    is_tuple_constructor_string uu____1028
...
let (mk_tuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____1045 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mktuple%s" uu____1045  in
      let uu____1048 = psnconst t  in FStar_Ident.set_lid_range uu____1048 r
...
let (is_tuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1076 = mk_tuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1076
...
let (mk_dtuple_lid : Prims.int -> FStar_Range.range -> FStar_Ident.lident) =
  fun n1  ->
    fun r  ->
      let t =
        let uu____1124 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "dtuple%s" uu____1124  in
      let uu____1127 = let uu____1128 = mod_prefix_dtuple n1  in uu____1128 t
         in
      FStar_Ident.set_lid_range uu____1127 r
...
let (mk_dtuple_data_lid :
  ...
  fun n1  ->
    fun r  ->
      let t =
        let uu____1169 = FStar_Util.string_of_int n1  in
        FStar_Util.format1 "Mkdtuple%s" uu____1169  in
      let uu____1172 = let uu____1173 = mod_prefix_dtuple n1  in uu____1173 t
         in
      FStar_Ident.set_lid_range uu____1172 r
...
let (is_dtuple_data_lid : FStar_Ident.lident -> Prims.int -> Prims.bool) =
  fun f  ->
    fun n1  ->
      let uu____1206 = mk_dtuple_data_lid n1 FStar_Range.dummyRange  in
      FStar_Ident.lid_equals f uu____1206
  
let (is_dtuple_data_lid' : FStar_Ident.lident -> Prims.bool) =
  fun f  ->
    let uu____1214 = FStar_Ident.text_of_lid f  in
    is_dtuple_datacon_string uu____1214
